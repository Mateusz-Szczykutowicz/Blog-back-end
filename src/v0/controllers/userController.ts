...
export = {
    user: {
        ...
        async getInfoAboutAccount(req: Request, res: Response) {
            const user = await User.findOne(
                { ID: req.body.id },
                "createdAt blocked login email signature"
            );
            if (!user) {
                return res
                    .status(404)
            ...
            return res
                .status(200)
                .json({ status: 200, message: "Status - success", user });
        },
    },
    admin: {
        ...
        async blockAccount(req: Request, res: Response) {
            ...
            if (user.get("admin")) {
                return res.status(403).json({
                    status: 403,
                    message: "Can't block administrator!",
                });
            }
            user.get("blocked")
